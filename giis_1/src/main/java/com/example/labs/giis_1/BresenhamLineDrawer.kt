...
object BresenhamLineDrawer : LineDrawer {
	...
	private fun Graphics.drawLineHigh(x1: Int, y1: Int, x2: Int, y2: Int) {
		var dx = x2 - x1 + 0f
		val dy = y2 - y1 + 0f
		var xi = 1
		if(dx < 0) {
	...
	private fun Graphics.drawLineLow(x1: Int, y1: Int, x2: Int, y2: Int) {
		val dx = x2 - x1 + 0f
		var dy = y2 - y1 + 0f
		var yi = 1
		if(dy < 0) {
