...
fun phi(x: BigInteger) = phi(x.toLong())
...
fun inverse(a: BigInteger, m: BigInteger) = a.modPow((phi(m) - 1).toBigInteger(), m)
...
fun main() {
	run {
		val p = BigInteger.valueOf(206181067L)
		val g = BigInteger.valueOf(7L)
		val y = BigInteger.valueOf(57348448L)
		val x = BigInteger.valueOf(92440205L)
		val w = BigInteger.valueOf(160936054L)
		val o = BigInteger.valueOf(144946434L)
		println("Decrypted Elgamal: ${(w * inverse(o.modPow(x, p), p)) % p}")
//		println("Decrypted Elgamal: ${decryptElgamal(w, o, y, p, g)}")
	}
	run {
		val w = BigInteger.valueOf(160936054L)
		...
		println("CRT X: ${crt(w, e, p, q)}")
	}
}

fun crt(w: BigInteger, e: BigInteger, p: BigInteger, q: BigInteger) = w.modPow(e, p * q)

fun decryptElgamal(w: BigInteger, o: BigInteger, y: BigInteger, p: BigInteger, g: BigInteger): BigInteger {
	val x = solveX(g, p, y)
	return (w * inverse(o.modPow(x, p), p)) % p
}

fun solveX(g: BigInteger, p: BigInteger, y: BigInteger): BigInteger {
